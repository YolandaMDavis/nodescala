...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      ...
      for(f <- fs) {
          f onComplete {
            tryValue => {if(!promise.isCompleted) promise.complete(tryValue)}
          }
       }
      promise.future
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = {
      try {
        ...
        case t: TimeoutException => throw new scala.NoSuchElementException()
      }
    }
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
        ...
        for{
          fs <- f
        } yield promise.complete(Try(cont(f)))
        promise.future
    }
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      val promise: Promise[S] = Promise[S]()
      f onComplete {
        tryValue => promise.complete(Try(cont(tryValue)))
      }
      promise.future
    }
