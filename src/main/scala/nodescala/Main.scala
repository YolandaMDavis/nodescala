...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val userInterrupted = Future.userInput("Hit ENTER to cancel... ") continue {
      f => "You entered... " + f.get + "\n"
    }
    ...
    terminationRequested onSuccess {
      case msg => print(msg); println("Bye!"); myServerSubscription.unsubscribe();
    }

    try {
      Await.result(terminationRequested, 30 seconds)
    } catch {
      case t: TimeoutException => println(t)
    }
